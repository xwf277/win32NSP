...
int main (int argc, char **argv)
{
	...
	int wargc;
	wchar_t **wargv;
	int nTest = sizeof(WSAQUERYSETW);
	fprintf (stderr, "WSAQUERYSETW SIZE =%d", sizeof(WSAQUERYSETW));
	...
	if (wargc == 5)
	{
		if (wcscmp (wargv[1], L"A") == 0)
			sc = const_cast<GUID*>(&SVCID_DNS_TYPE_A);
		else if (wcscmp (wargv[1], L"AAAA") == 0)
			sc = const_cast<GUID*>(&SVCID_DNS_TYPE_AAAA);
		else if (wcscmp (wargv[1], L"name") == 0)
			sc = const_cast<GUID*>(&SVCID_HOSTNAME);
		else if (wcscmp (wargv[1], L"addr") == 0)
			sc = const_cast<GUID*>(&SVCID_INET_HOSTADDRBYNAME);
		else
			wargc -= 1;
		if (wcscmp (wargv[4], L"mswdns") == 0)
			prov = const_cast<GUID*>(&W32_DNS);
		else if (wcscmp (wargv[4], L"gnunetdns") == 0)
			prov = const_cast<GUID*>(&GNUNET_NAMESPACE_PROVIDER_DNS);
		else
			wargc -= 1;
	}
			...
			if (startup != NULL)
			{
				...
				else
				{
					...
					else
					{
						...
						if (ret != NO_ERROR)
						{
							fprintf (stderr, "lookup next failed! err=%d\n", err);
						}
						else
						{
							int i = 0;
							...
							printf ("  lpBlob:                  0x%p\n", result->lpBlob);
							printf ("  lpResult:                  0x%p\n", result);
							if (result->lpBlob)
							{
								struct hostent *he = (hostent*)malloc(result->lpBlob->cbSize);
								if (he != NULL)
								{
